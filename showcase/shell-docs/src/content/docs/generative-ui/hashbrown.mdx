---
title: Declarative UI — Hashbrown
...
If you'd rather work with an explicit catalog of registered components rather than streaming a JSON tree, see the sibling page [Declarative UI — JSON Render](/generative-ui/json-render) for the same scenario implemented with `@json-render/react`.
...
Both `declarative-hashbrown` and [`declarative-json-render`](/generative-ui/json-render) solve the same problem (agent-authored structured UI, rendered through a typed catalog), with two different rendering libraries. Pick whichever you already use elsewhere — the agent contract is the same shape; the React glue is what changes.
