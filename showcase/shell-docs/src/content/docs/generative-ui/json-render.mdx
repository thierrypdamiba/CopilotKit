---
title: Declarative UI — JSON Render
...
If you'd rather have a streaming progressive render rather than a one-shot validated render, see the sibling page [Declarative UI — Hashbrown](/generative-ui/hashbrown) for the same scenario with `@hashbrownai/react`.
...
Both `declarative-json-render` and [`declarative-hashbrown`](/generative-ui/hashbrown) solve the same problem with two different rendering libraries. The agent contract is similar; the React glue, validation strategy, and rendering behaviour differ.
