        ...
        todos.append(
            {
                ...
                "status": (
                    "completed"
                    if raw_todo.get("status") == "completed"
                    else "pending"
                ),
            }
        )
...
def _execute_tool(
    ...
    if name == "get_stock_price":
        return json.dumps(_get_stock_price_impl(str(tool_input.get("ticker", "")))), None

    if name == "roll_d20":
        value = tool_input.get("value")
        return json.dumps(
            {"value": int(value) if isinstance(value, (int, float)) else random.randint(1, 20)}
        ), None
...
async def run_agent(
    ...
    *,
    ...
    sdk_backend_tools = [] if disable_tools else (
        tools_override if tools_override is not None else TOOLS
    )
