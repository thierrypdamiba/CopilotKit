...
import {
  ...
  queryDataImpl,
  renderFlightsImpl,
} from "./agent/beautiful-chat-tools";
...
const CLAUDE_VISION_MODEL = process.env.CLAUDE_VISION_MODEL || CLAUDE_MODEL;
...
function normalizeA2uiComponents(
  ...
  for (const component of sanitized) {
    ...
    if (seenIds.has(id)) {
      console.warn(
        `[agent_server] dropping duplicate A2UI component id=${id}`,
      );
      continue;
    }
    seenIds.add(id);
  ...
  if (topLevelComponents.length === 1) {
    ...
    console.warn(
      `[agent_server] normalizing A2UI root id ${priorId} -> root`,
    );
